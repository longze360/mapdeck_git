...
addDependency <- function(map, dependencyFunction, priority = FALSE) {
  ...
  if (!addingDependency %in% existingDeps) {
    map$dependencies <- if (priority) {
      c(dependencyFunction, map$dependencies)
    } else {
      c(map$dependencies, dependencyFunction)
    }
  }
...
# Leaflet dependencies for leaflet map_type
#' @export
leafletDependency <- function() {
  list(
    createHtmlDependency(
      name = "leaflet",
      version = "1.9.4",
      src = system.file("htmlwidgets/lib/leaflet", package = "mapdeck"),
      script = c("leaflet.js"),
